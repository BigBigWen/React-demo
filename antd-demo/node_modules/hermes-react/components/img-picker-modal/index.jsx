...
class ImgPickerModal extends Component {
  static propTypes = {
    // 选择部分
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      data: [], // 图片数据
      selectedMap: this.props.selected.reduce((memo, id) => {
        return { ...memo, [id]: true };
      }, {}), // 选中内容
  ...
  beforeUpload = (file) => {
    ...
    if (errorFlag && !this.isPromise(errorFlag)) { // 如果是promise格式则不能判断loading状态
  ...
  getPics = () => {
    ...
    return data.map((row, i) => {
      return (
        <div key={i} className="hermes-photo-picker-list-item" onClick={this.selectPhoto.bind(this, row.id, row)}>
          {lazyLoad
            ? <LazyImg container={this.picContainer} src={row.thumbUrl ? row.thumbUrl : row.url} alt="img" placeholder="" />
            : <img src={row.thumbUrl ? row.thumbUrl : row.url} alt="img" />}
          {selectedMap[row.id] && (<div className="hermes-photo-picker-list-item-icon">
  ...
  render() {
    const { multiple, min, uploadUrl, notice, onPageChange = noop, uploadNotice } = this.props;
    const { data, loading, selectedMap, picLoading, visible, pagination } = this.state;
    const selectedCount = Object.keys(selectedMap).length;

    return (<div>
      <Modal visible={visible} width={711} zIndex={1000} footer="" onCancel={this.onCancel} {...this.props}>
        {uploadUrl ? <div className="hermes-photo-picker-header">
          <div className="hermes-photo-picker-upload">
            <Upload {...this.uploadProps()} >
              {uploadNotice ?
                <Popover placement="topRight" content={uploadNotice}><Button size="large" type="ghost" loading={loading}>上传新图片</Button></Popover> :
                <Button size="large" type="ghost" loading={loading}>上传新图片</Button>
              }
            </Upload>
          </div>
        </div> : null}
        <LazyContainer className={`hermes-photo-picker-list ${pagination && pagination.total ? ' hermes-photo-picker-pagination-list' : ''}`} ref={dom => this.picContainer = dom}>
          {picLoading ? <div style={{ textAlign: 'center', lineHeight: '280px' }}><Spin /></div> : <div>
            {!(data && data.length) ? <div style={{ textAlign: 'center', lineHeight: '280px' }}>暂无图片</div> : this.getPics()}
          </div>}
        </LazyContainer>
        <div className="hermes-photo-picker-footer">
          {notice ? <p className="notice-info">{notice}</p> : null}
          {pagination && pagination.total ? <div className="hermes-photo-picker-pagination"><Pagination
            {...pagination}
            onChange={(num) => {
              this.setState({
                picLoading: true,
