...
import Tree from './node-model';
import { DEFAULT_ASYNC_NODE_TEXT, DEFAULT_NO_FOUND_CONTENT, toggle, DEFAULT_ROOT } from './utils';
...
class AsyncTree extends Component {
  ...
  componentWillReceiveProps(props) {
    ...
    if (this.props.searchValue !== searchValue) {
      if (searchValue) {
        ...
        let searchModel = new Tree({ ...DEFAULT_ROOT }, {
          fetch: (node) => {
  ...
  onExpand(id, isRight) {
    const { model, searching, searchModel, searchExpandKey, leftExpandKey, rightExpandKey } = this.state;
    if (isRight) {
      this.setState({
  ...
  getRenderInfo(isRight) {
    const { model, searching, searchModel, rightModel, searchExpandKey, leftExpandKey, rightExpandKey } = this.state;
    let tree;
    let expandIds;
    if (isRight) {
      tree = rightModel;
  ...
  reset() {
    ...
    ps.push(model.fetch());
    if (searchValue) {
      searchModel = new Tree({ ...DEFAULT_ROOT }, {
        fetch(node) {
          return fetch(node, searchValue);
        },
        async: true,
        ...
        <div className="header">
          {
            rootDegree && showCheckAll ? (
              <Node node={tree} onCheck={this.onCheck.bind(this, '#', false)} nodeText={() => undefined} />
            ) : false
          }
